...
#define CT_ERROR(_rc, _format, ...) _trace(_rc, ERR, #_format, ## __VA_ARGS__)

#define CT_WARN(_format, ...)       _trace(0, WARN, #_format, ## __VA_ARGS__)

#define CT_TRACE(_format, ...)      _trace(0, TRACE, #_format, ## __VA_ARGS__)
    ...
    switch (trig) {
      case 'A':
        if ((cpt_opt.arch_ind_count >= MAX_ARCH)
            ...
            ret = -E2BIG;
            CT_ERROR(ret, "Archive number must be less than %zu.", MAX_ARCH);
            return (ret);
        }
...
static int      cpt_get_lustre_path(char *buf, int sz,
                                    const char *mnt, const lustre_fid *fid) {
...
static int      cpt_begin(struct hsm_copyaction_private **phcp,
                          const struct hsm_action_item *hai,
                          int mdt_index, int open_flags) {
  int           ret;
  char          src[PATH_MAX];

  ret = llapi_hsm_action_begin(phcp, cpt_data, hai,
...
static int                       cpt_end(struct hsm_copyaction_private **phcp,
                                         const struct hsm_action_item *hai,
                                         int hp_flags, int cpt_ret) {
  ...
  if (ret == -ECANCELED)
    ...
    CT_TRACE("llapi_hsm_action_end() ok (ret=%d)", ret);
  return (ret);
}
...
dpl_dict_t              *archive_attr(int src_fd, const char *src) {
  ...
  xattr_size = fgetxattr(src_fd, XATTR_LUSTRE_LOV, lov_buff, sizeof(lov_buff));
  if (xattr_size < 0) {
      ret = -EINVAL;
...
int                     restore_attr(dpl_dict_t *dict_var, int lustre_fd) {
  ...
  ret = fsetxattr(lustre_fd, XATTR_LUSTRE_LOV, buff, xattr_size, XATTR_CREATE);
  if (ret < 0) {
...
static int                       archive_data(const struct hsm_action_item *hai,
...
const                                         BIGNUM *bn) {
  ...
  if (src_st.st_size <= 0) {
      CT_TRACE("The specified file has no data."
               " Only xattrs will be archived on the Ring.\n"
               "If your file was supposed to contain data,"
               " please remove from the Ring and "
  ...
  dpl_ret = dpl_put_id(ctx, NULL, bnhex, &dpl_opts, DPL_FTYPE_REG, NULL,
                       NULL, dict_var, NULL, buff_data, src_st.st_size);
  if (dpl_ret != DPL_SUCCESS) {
      ret = -errno;
      CT_ERROR(ret, "DPL_PUT_ID failed for operation archive data = '%s'.",
...
static int                       restore_data(const struct hsm_action_item *hai,
                                              const long hal_flags,
                                              dpl_ctx_t *ctx,
                                              const BIGNUM *bn) {
  char                          *buff_data = NULL;
  ...
  dpl_ret = dpl_get_id(ctx, NULL, bnhex, &dpl_opts, DPL_FTYPE_REG,
                       NULL, NULL, &buff_data, &lenp, &dict_var, NULL);
  if (dpl_ret != DPL_SUCCESS) {
      ret = -EINVAL;
  ...
  if (!dict_var) {
      CT_ERROR(ret, "Cannot set lov xattr on '%s' for operation"
                    " restore data, using defaults.",
               lustre_fd);
      set_lovea = false;
  } else {
    set_lovea = true;
  ...
  if ((ret2 = llapi_file_fget_mdtidx(cpt_opt.lustre_mp_fd, &mdt_index)) < 0) {
  ...
  if (ret2 < 0) {
      CT_ERROR(ret2, "restoring "DFID" , cannot get FID"
               " of created volatile file", PFID(&hai->hai_fid));
      goto end;
  }
  ...
  if (set_lovea) {
      ret2 = restore_attr(dict_var, lustre_fd);
  ...
  if (ret2 < 0) {
      CT_ERROR(ret, "Couldn't properly write on Lustre's"
               " fd for action restore.");
      ret = ret2;
      goto end;
  }
...
static int                       remove_data(const struct hsm_action_item *hai,
                                             const long hal_flags,
                                             dpl_ctx_t *ctx,
                                             const BIGNUM *bn) {
  char                           bnhex[DPL_UKS_BCH_LEN + 1];
  ...
  if (dpl_ret != DPL_SUCCESS) {
      ret = -EINVAL;
      CT_ERROR(ret, "Dpl_delete_id failed for operation remove data = '%s.",
               dpl_status_str(dpl_ret));
      goto end;
  } else {
    ret = 0;
  ...
  // FIXME attr ret fails so debug set to 0
...
static int      process_action(const struct hsm_action_item *hai,
                               const long hal_flags, dpl_ctx_t *ctx,
                               const BIGNUM *bn) {
  int           ret, ret2;
  char          fid[128];
  char          path[PATH_MAX];
...
BIGNUM          *uks_key_from_fid(int64_t f_seq, int32_t f_oid, int32_t f_ver) {
  uint32_t      BN_hash;
  int           ver_lsb;
  BIGNUM       *bn;
...
static void             *cpt_thread(void *data) {
...
static int              process_async(const struct hsm_action_item *hai,
                                      long hal_flags, dpl_ctx_t *ctx,
                                      const BIGNUM *bn) {
  pthread_attr_t        attr;
  pthread_t             thread;
  ...
  if ((ret = pthread_attr_init(&attr)) != 0) {
      CT_ERROR(ret, "pthread_attr_init failed for '%s' service.",
               cpt_opt.lustre_mp);
      if (data->hai)
        free(data->hai);
      free(data);
...
static int                      cpt_run(dpl_ctx_t *ctx) {
  ...
  while (1) {
      ...
      while (++i <= hal->hal_count) {
          ...
          if (!(bn = uks_key_from_fid(hai->hai_fid.f_seq,
                hai->hai_fid.f_oid, hai->hai_fid.f_ver)))
            return (-ENOMEM);
          tmp = BN_bn2hex(bn);
